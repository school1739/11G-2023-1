...
"""
# задаем размер экрана и говорим где его центр
width = 1500
height = 700
...
class Balloon:  # Класс для шариков
    def balloon(point):
...
for i in range(10): # Просто делаем 10 шариков
    Balloon.balloon(sd.random_point())

class Firework:  # Класс для фейерверков
    def fireworks(x, y):
        sd.circle(sd.get_point(x, y), 10, sd.random_color(), 0)
class FireworkCenter(Firework):  # Вроде как подкласс для большей части фейерверка
    for i in range(1000):
        Firework.fireworks(center.x + random.randint(-150, 150), center.y + random.randint(-150, 150))
class FireworkAround(Firework):  # Вроде как подкласс для маленьких фейерверков
    for indent in range(3):  # Отступ между салютами
        for i in range(300):
            Firework.fireworks(center.x + random.randint(-450, -350) + 400 * indent,
                               center.y + random.randint(250, 350))
    for indent in range(2):  # Еще один отступ между салютами
        for i in range(300):
